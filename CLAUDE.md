...
make docker-up              # 后台起容器（挂仓库根 config-docker.yaml）
make docker-down            # 停并删除容器
make docker-logs            # 跟踪日志
```

产物在 `build/<os>_<arch>/sb-sub-c`。Air 热重载开发可用 `.air.toml`。本地 Docker 的镜像 / 容器 / 端口取值见 `Makefile` 顶部的 `LocalImage` / `LocalContainer` / `LocalPort`（端口须与所挂载 `config-docker.yaml` 的 `server.port` 一致）。
